...
//   2. generateActivationNumber({...})       → "K7QML3F-8AZ…"      (21 chars)
...
//
//   plaintext (10 bytes)
//     [0..3)   machineId(3)   first 3 bytes of the Machine ID (6 hex chars)
//     [3..5)   startDays      uint16   (days since 2025-01-01)
//     [5..7)   expiryDays     uint16
//     [7..10)  licenseNumber  uint24   (0 .. 99999, 5-digit reference)
//
//   keys   = SHA256(secret | "enc"/"mac")
//   tag    = HMAC(macKey, plaintext)[0..3]   ← tamper seal AND the IV
//   cipher = AES-256-CTR(encKey, iv=tag).encrypt(plaintext)
//   wire (13 bytes) = tag(3) + cipher(10)
//   activation number = Base32(wire), grouped in 7s  → 21 characters
// ─────────────────────────────────────────────────────────────────────────────
...
const TAG_BYTES = 3;                  // 24-bit tamper tag (also the CTR IV seed)
const MACHINE_ID_BYTES = 3;           // store first 3 bytes (6 hex chars) — 24-bit machine bind
const LICENSE_BYTES = 3;              // uint24 license number (5-digit reference)
const MAX_LICENSE_NUMBER = 99999;     // enforce 5-digit rule
const PLAINTEXT_BYTES = MACHINE_ID_BYTES + 2 + 2 + LICENSE_BYTES; // 10
const WIRE_BYTES = TAG_BYTES + PLAINTEXT_BYTES;                   // 13
const KEY_GROUP_SIZE = 7;             // 21 chars → 3 clean groups of 7
 ...
 * @param {number} a.licenseNumber  integer 0 .. 99999 (5-digit reference)
 * @param {Buffer} a.secret         shared secret
 * @returns {string} grouped activation number (21 chars)
 */
function generateActivationNumber({ machineId, startDate, expiryDate, licenseNumber, secret }) {
  ...
  if (!Number.isInteger(licenseNumber) || licenseNumber < 0 || licenseNumber > MAX_LICENSE_NUMBER) {
    throw new Error('licenseNumber must be an integer 0 .. ' + MAX_LICENSE_NUMBER);
  }

  const plaintext = Buffer.alloc(PLAINTEXT_BYTES);
  Buffer.from(id, 'hex').copy(plaintext, 0, 0, MACHINE_ID_BYTES);  // first 3 bytes of the ID
  ...
  plaintext.writeUIntBE(licenseNumber, MACHINE_ID_BYTES + 4, LICENSE_BYTES); // [7..10) uint24

  const { encKey, macKey } = deriveKeys(secret);
  const tag = tagOf(macKey, plaintext);
  const cipher = ctr(encKey, tag, plaintext);

  return groupChars(base32Encode(Buffer.concat([tag, cipher])), KEY_GROUP_SIZE);
}
...
function readActivationNumber(activationNumber, secret) {
  ...
  return {
    valid: true,
    machineId: plaintext.subarray(0, MACHINE_ID_BYTES).toString('hex').toUpperCase(), // 6 hex chars
    startDate: daysToDate(plaintext.readUInt16BE(MACHINE_ID_BYTES)),                   // [3..5)
    expiryDate: daysToDate(plaintext.readUInt16BE(MACHINE_ID_BYTES + 2)),              // [5..7)
    licenseNumber: plaintext.readUIntBE(MACHINE_ID_BYTES + 4, LICENSE_BYTES),          // [7..10)
  };
}
